...
export function MessageList() {
  ...
  const vigilRef = useRef<HTMLLIElement>(null!);
  const mountRef = useRef<boolean>(false);
  const [messages, setMessages] = useState<MessageModel[]>([]);

  const fetch = async (pointer: number) => {
    const { data } = await api.get<MessageModel[]>("messages", {
      params: { cursor: pointer },
    });
    cursor = data[data.length - 1]?.id || cursor;
  ...
  useEffect(() => {
    ...
    const iObserver = new IntersectionObserver((entries) => {
      if (entries[0].isIntersecting && mountRef.current) {
        fetch(cursor - 1);
      }
      mountRef.current = true;
    });

    iObserver.observe(vigilRef.current);
    ...
    setInterval(() => {
      if (messagesQueue.length > 0) {
        setMessages((prevMessages) =>
          [messagesQueue[0], ...prevMessages].filter(Boolean)
        );
        ...
        <ul className={styles.messageList}>
          {messages.map((message) => {
            return (
              <li
                key={message.id}
                className={`${styles.message} ${styles.anim}`}
              >
          ...
          <li className={styles.vigil} ref={vigilRef}></li>
        </ul>
      </div>
    </section>
  );
